...
class Parser:
  ...
  def __init__(self,toks,image_path):
    ...
    self.glo["PC"]=Token(0,0,TokenType.INTEGER,0)

    self.glo["DBG"]=Token(0,0,TokenType.INTEGER,0)
    self.glo["RET"]=Token(0,0,TokenType.LIST,[])
    self.glo["STK"]=Token(0,0,TokenType.LIST,[])
  ...
  def get_ident(self):
    ...
    for _ in range(count):
      if "VAR_"+res.value not in self.glo:
        self.error(f"undefined identifier {res.value}")
      res=self.glo["VAR_"+res.value]
  ...
  def get_any(self):
    ...
    result=Token(0,0,TokenType.NONE,None)
    if self.get_type() in [TokenType.IDENT,TokenType.AT]:
      tok0=self.get_ident()
      result=self.glo["VAR_"+tok0.value]
  ...
  def do_set(self):
    ...
    tok0=Token(0,0,TokenType.NONE,None)
    if self.get_type()==TokenType.STRING:
      tok0=self.get_string()
    elif self.get_type() in [TokenType.IDENT,TokenType.AT]:
      tok0=self.get_ident()
    else:
      self.error(f"found {self.get_type()} expected STRING or IDENT")

    tok1=self.get_any()
    while self.get_type() not in [TokenType.SEMI_COLON,TokenType.NEW_LINE,TokenType.EOF]:
      tok1=Token(0,0,TokenType.STRING,str(tok1.value)+str(self.get_any().value))
    self.glo["VAR_"+tok0.value]=tok1        
    self.end_tag("do_set") 
  ...
  def do_ret(self):
    ...
    self.pc=self.glo["RET"].pop().value
    self.end_tag("do_ret") 
  ...
  def do_jmp(self):
    ...
    tok0=self.get_label_check()
    self.pc=self.glo["LAB_"+tok0.value].value
    self.end_tag("do_jmp") 
  ...
  def do_push(self):
    ...
    res=Token(0,0,TokenType.NONE,None)
    while self.get_type() not in [TokenType.SEMI_COLON,TokenType.NEW_LINE,TokenType.EOF]:
      if self.get_type() in [TokenType.IDENT,TokenType.AT]:
        tok0=self.get_ident()
        res=self.glo["VAR_"+tok0.value]
      else:
        res=self.get_token()
        self.next()
      self.glo["STK"].append(res)
